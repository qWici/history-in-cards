...
| Режим | URL | Опис |
|---|---|---|
| Класичний | `/play` | Всі категорії впереміш, 3 життя, рахунок = серія правильних. 4 рівні складності (екран вибору перед грою): 🌱 легкий — топ-30% найвідоміших карток, 🎯 звичайний — повна крива, 🔥 важкий — без топ-20%, 🎓 історик — нижні 55% за відомістю. У кожного рівня свій рекорд |
...
(наступник PartyKit від Cloudflare). Складність кімнати обирає хост у лобі
(ті самі 4 рівні, що й у класиці) — колоду будує сервер, тож рівень чесний
для всіх. Авторитарний сервер у `party/room.ts`,
...
правильні/неправильні ходи, для класики — рівень складності) → лічильники в
Upstash Redis. `GET /api/stats` — публічна агрегація (кеш CDN 60с), сторінка
`/stats` малює гістограми: загальну, розбивку класики за рівнями і щоденний.

Ключі Redis: `stats:v1:games` (лічильники режимів), `stats:v1:moves`,
гістограми `stats:v1:hist:<mode>`; рівні класики — `hist:classic:easy` /
`hist:classic:hard` / `hist:classic:historian`, а «звичайний» пише у легасі
`hist:classic` — там же лишаються всі партії, зіграні до появи рівнів.
